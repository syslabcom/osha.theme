...
function LoadSelection()
{
    ...
    if( onfocus )
    {
        ...
        if (ofv == 'adjustTitle(this)') {
            ...
            if( ofv.indexOf('{') != -1 )
                ofv = ofv.substring( (ofv.indexOf('{')+1), ofv.indexOf('}') );
            GetE('txtAdjustWAITitle').value = ofv.split('(', 1)[0];
        }
    }
...
function SetTarget( targetType )
{
	...
	// XXX Changes by SLC:
	GetE('tableWAIFeatures').style.display = ( targetType == 'wai_blank' ? '' : 'none' ) ;
	GetE('tdTargetFrame').style.display	= ( targetType == 'wai_blank' ? 'none' : '' ) ;
...
function Ok()
{
	...
	for ( var i = 0 ; i < aLinks.length ; i++ )
	{
        ...
        if( GetE('cmbTarget').value == 'wai_blank' ) {
            ...
            if (eventHandler != '') {
                SetAttribute (oLink, 'onfocus', eventHandler+'(this)' );
                SetAttribute (oLink, 'onmouseover', eventHandler+'(this)');
            }
            else {
                oLink.removeAttribute('onfocus');
                oLink.removeAttribute('onmouseover');
            }
        }
		else if( GetE('cmbTarget').value != 'popup' ) {
